...
int main()
{
    ...
    // Вектор для хранения адресов выделенной нами памяти (чтобы было удобно освобождать)
    ...
    do {
        ...
        case '2': {
            ...
            // Установка 2-ух знаков после запятой
        ...
        case '4': {
            ...
            // Этап 1: Резервирование
            LPVOID reservedAddr = VirtualAlloc(targetAddr, size, MEM_RESERVE, PAGE_READWRITE);
            if (!reservedAddr) {
                ...
                // Этап 2: Передача физической памяти (Commit)
                LPVOID committedAddr = VirtualAlloc(reservedAddr, size, MEM_COMMIT, PAGE_READWRITE);
                if (!committedAddr) {
        ...
        case '5': {
            ...
            LPVOID allocatedAddr = VirtualAlloc(targetAddr, size, MEM_RESERVE | MEM_COMMIT, PAGE_READWRITE);
            if (!allocatedAddr) {
        ...
        case '7': {
            ...
            // Устанавливаем защиту "Только чтение" (PAGE_READONLY)
            if (VirtualProtect(reinterpret_cast<LPVOID>(inputAddr), size, PAGE_READONLY, &oldProtect)) {
                ...
                // Возвращаем старую защиту, чтобы потом можно было освободить/использовать память
                DWORD temp;
                VirtualProtect(reinterpret_cast<LPVOID>(inputAddr), size, oldProtect, &temp);
        ...
        case '8': {
            // Дополнительный пункт для очистки выделенной нами памяти
            if (myAllocations.empty()) std::wcout << L"\nНет выделенной памяти для освобождения.\n";
            else {
                std::wcout << L"\nДоступные адреса для освобождения:\n";
                for (size_t i=0; i < myAllocations.size(); i++) {
                    std::wcout << i + 1 << L". 0x" << std::hex << myAllocations[i] << std::dec << L"\n";
                }
                std::wcout << L"\nВведите номер (или 0 для отмены): ";
                std::size_t delChoice;
                std::wcin >> delChoice;

                if (delChoice > 0 && delChoice <= myAllocations.size()) {
                    if (VirtualFree(myAllocations[delChoice - 1], 0, MEM_RELEASE)) {
                        std::wcout << L"\n[+] Память успешно освобождена!\n";
                        myAllocations.erase(myAllocations.begin() + (delChoice - 1));
                    }
                    else Error(4);
                }
            }
...
void MenuOutput() {
    ...
    std::wcout << L"8 - Освободить выделенную память (VirtualFree)\n";
    std::wcout << L"0 - Выход\n";
    std::wcout << L"\nВыберите пункт: ";
}
...
std::wstring GetProtectString(DWORD protect) {
    // Сбрасываем флаги-модификаторы для чистоты
...
void Error(int type) {
    ...
    case 4: std::wcout << L"\nОшибка VirtualFree. Код: " << error << std::endl; break;
